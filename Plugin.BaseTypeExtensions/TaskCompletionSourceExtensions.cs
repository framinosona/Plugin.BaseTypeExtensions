...
public static class TaskCompletionSourceExtensions
{
    ...
    /// }
    /// </code>
    public async static Task WaitAndFossilizeTaskOnTimeoutAsync(this TaskCompletionSource tcs, TimeSpan timespan, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(tcs);
        ...
        try
        {
            await tcs.Task.WaitAsync(timespan, cancellationToken).ConfigureAwait(false);
        }
    ...
    /// }
    /// </code>
    public async static Task<T> WaitAndFossilizeTaskOnTimeoutAsync<T>(this TaskCompletionSource<T> tcs, TimeSpan timespan, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(tcs);
        if (timespan < TimeSpan.Zero) //note that this deviates from the behaviour of .WaitAsync() which does accept -1 milliseconds which means "wait forever"
        {
        ...
        try
        {
            return await tcs.Task.WaitAsync(timespan, cancellationToken).ConfigureAwait(false);
        }
        catch (Exception ex) when (ex is TimeoutException or OperationCanceledException or TaskCanceledException) //taskcanceledexception and operationcanceledexception can come from cancellation-token timeouts
        {
            var properExceptionToThrow = DeduceProperExceptionToThrow(ex, cancellationToken);
            ...
            return await tcs.Task.ConfigureAwait(false); //this means the task itself faulted
        }
    ...
    private static Exception DeduceProperExceptionToThrow(Exception ex, CancellationToken cancellationToken)
    {
